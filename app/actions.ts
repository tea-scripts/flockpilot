...
function buildDemoRequestEmail(payload: DemoRequestPayload): { html: string; text: string } {
  ...
  const html = `
    <div style="margin:0; padding:24px; background:#0E2D1D; font-family:Arial, Helvetica, sans-serif;">
      <table role="presentation" cellspacing="0" cellpadding="0" width="100%" style="max-width:760px; margin:0 auto; border:1px solid #0E2D1D; border-radius:14px; overflow:hidden; background:#f7f9f4;">
        <tr>
          <td style="padding:20px 24px; background:linear-gradient(90deg, #0E2D1D 0%, #32A365 100%); color:#ffffff;">
